...
  // This effect updates the displayed time for the live video stream
  useEffect(() => {
    if (isStreamReady && currentStore) {
      const timer = setInterval(() => {
        setLiveStreamTime(formatTimeInZone(new Date(), currentStore.timezone));
      }, 1000);
      return () => clearInterval(timer);
    }
  }, [isStreamReady, currentStore]);

  // This is the main effect for setting up the camera feed (polling and WebRTC)
  useEffect(() => {
...
      if (isCancelled || !currentStore) return;
      try {
...
        console.error("Failed to load preview image during polling", e);
...
        setError("Live stream failed. Displaying latest snapshot.");
        if (pollingInterval) clearInterval(pollingInterval); 
        setIsLoading(false);
      }
    };
    ...
    const startProcess = async () => {
      setIsLoading(true);
      await fetchStaticImage();
      setIsLoading(false);
      if (isCancelled) return;
      
      pollingInterval = setInterval(fetchStaticImage, 1500);
      setupStream();
    };
...
  const isUpgrading = previewUrl && !isStreamReady && !error;
  const displayTimestamp = isStreamReady 
    ? liveStreamTime 
    : previewTimestamp && currentStore
      ? formatTimeInZone(previewTimestamp, currentStore.timezone)
      : '';
...
        {isStreamReady && (
          <div className="absolute top-3 left-3 flex items-center space-x-2 bg-black/50 text-white px-3 py-1 rounded-full text-xs font-bold backdrop-blur-sm">
