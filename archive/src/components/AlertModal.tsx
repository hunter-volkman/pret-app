        ...
        <div className="p-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm bg-gray-50 rounded-lg p-4">
              ...
              <div><span className="text-gray-600">Time:</span><span className="ml-2 font-medium">{new Date(alert.timestamp).toLocaleString()}</span></div>
              <div><span className="text-gray-600">Type:</span><span className="ml-2 font-medium capitalize">{alert.type}</span></div>
              <div>
                <span className="text-gray-600">Severity:</span>
                <span className={`ml-2 font-medium capitalize ${alert.severity === 'high' ? 'text-red-600' : 'text-yellow-600'}`}>
                  {alert.severity}
                </span>
              </div>
            </div>
                    ...
                    {imageToDisplay ? (
                      <>
                        ...
                        <div className="absolute bottom-3 right-3 bg-black/60 text-white px-3 py-1 rounded-md text-sm font-mono backdrop-blur-sm">
                          {store ? formatTimeInZone(alert.timestamp, store.timezone) : alert.timestamp.toLocaleTimeString()}
                        </div>
                      </>
                    ) : (
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  {alert.sensors.map((sensor) => {
                    const config = store ? getSensorConfig(store.tempMachineId, sensor.id) : null;
                    const name = config?.displayName || sensor.id;
                    const status = alert.severity === 'high' ? 'critical' : 'warning';
                    return (
                      <div key={sensor.id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
            ...
            <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
              {!alert.read && (
                <button onClick={() => { onMarkRead(); onClose(); }} className="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">
                  Mark as Read
                </button>
              )}
              <button onClick={onClose} className="bg-gray-100 text-gray-700 px-6 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">
                Close
              </button>
            </div>
          </div>
