...
// This provides modularity for our rules.
const SENSOR_TYPES = {
  FRIDGE: {
    displayName: 'Fridge',
    // Rules for instantaneous temperature status. All values are in Celsius.
    thresholds: {
      critical: 5,  // Temp > 5°C is critical
      warning: 1,   // Temp < 1°C is a warning (too cold)
    },
    // Configuration for the compliance bands on the chart.
    bands: [
      { y: 5, label: 'Critical Upper Limit', color: '#EF4444' },
      { y: 1, label: 'Warning Lower Limit', color: '#F59E0B' },
    ],
    // Time-based alerting rules for the MonitorService (Phase 3)
    alerting: {
      upperLimit: 5,
      firstAlertMinutes: 45,
      secondAlertMinutes: 90, // 45 + 45
    },
  },
  LANGAR: {
    displayName: 'Langar',
    thresholds: {
      critical: 8,  // Temp > 8°C is critical
      warning: 1,   // Temp < 1°C is a warning
    },
    bands: [
    ...
    alerting: {
      upperLimit: 8,
      firstAlertMinutes: 45,
      secondAlertMinutes: 90,
    },
  },
  FREEZER: {
    displayName: 'Freezer',
    thresholds: {
      critical: -17.8, // Temp > -17.8°C (0°F) is critical
      warning: -22,    // Temp < -22°C is a warning (too cold)
    },
    bands: [
      { y: -17.8, label: 'Critical Upper Limit (0°F)', color: '#EF4444' },
      { y: -22, label: 'Warning Lower Limit', color: '#F59E0B' },
    ],
    alerting: {
      upperLimit: -17.8,
      firstAlertMinutes: 45,
  ...
  AMBIENT: {
    ...
    bands: [], // No bands for ambient sensors for now
    alerting: null, // No alerts for ambient sensors
  },
};

// 2. Map the raw sensor names from Viam to a type and display name for each store.
// This provides store-specific flexibility.
const STORE_SENSOR_MAPPINGS: Record<string, Record<string, { type: keyof typeof SENSOR_TYPES; displayName: string }>> = {
  ...
  '948e0595-d307-425f-bd55-108e52046c2b': {
    ...
    'sensor-3': { type: 'LANGAR', displayName: 'Pastry Display' }, // Example of a Langar
  ...
  '27971efd-baaa-4cb5-b5d0-2b5468daf0b4': {
    ...
    'sensor-7': { type: 'LANGAR', displayName: 'Pastry Case' }, // Example of a Langar
